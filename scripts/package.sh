...
asset_name="JiuyiTranslate-$version-macOS-$BUILD_ARCH"
staging="$(mktemp -d "$PWD/dist/.dmg-build.XXXXXX")"
trap 'rm -rf "$staging"' EXIT
ditto "$app_path" "$staging/啾译.app"
...
hdiutil create -volname "啾译 $version" -srcfolder "$staging" -format UDZO -ov "dist/$asset_name.dmg"

...
> **当前为 v1.0.0 预览版，使用临时签名，尚未经过 Apple 公证。** 首次打开可能被 macOS 阻止；请先阅读下方安装说明。本项目不是 Bob 的官方版本，与 Bob 无关联。
...
| 自定义快捷键 | 设置 → 翻译快捷键，按下新组合键即可；需包含 ⌘、⌥ 或 ⌃ |
...
| 隐藏菜单栏图标 | 在设置关闭图标后，⌥D 仍可使用；Dock 图标仍显示 |
...
先用鼠标选中网页文字，再按 ⌥D 并松开。取词期间不要切换应用或另行复制。程序优先读取辅助功能选区，失败后发送一次 ⌘C 尝试读取选区。浏览器跨段落的长选区常被压成一行，这时同样改用复制取词以保留换行。受限制网页、图片或扫描 PDF 仍可能无法取词。也可先复制，然后在翻译框粘贴。
...
按 ⌥D 打开浮窗，再点右上角齿轮；也可以点击 Dock 图标打开翻译窗口。隐藏菜单栏图标不会退出应用，自启动开关独立设置。
